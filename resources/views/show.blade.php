    ...
    <div class="grid md:grid-cols-4 mt-8 ml-32 mr-8">        
        <main class="px-16 py-6 pb-2 md:col-span-3  bg-gray-200 dark:bg-slate-900">                 
            <div class=" grid md:grid-cols-2 pl-8 rounded ">
                ...
                <div class="pb-2 text-lg text-blue-400 font-bold">{{ $post->name }}</div>
                <div class="pb-2 txt-md font-semibold italic">Category:</div> 
                <div class="pb-2 text-lg text-blue-400 font-bold">{{ $post->category }}</div>
                <div class="pb-2 txt-md font-semibold italic">Description:</div>
                <div class="pb-2 text-lg text-blue-400 font-bold">{{ $post->description }}</div>
                <div class="pb-2 txt-md font-semibold italic">Price:</div>
                <div class="pb-2 italic text-2xl text-blue-400 font-bold">${{ $post->price }}</div>
                <div class="pb-8 txt-md font-semibold italic">Posted by:</div>
                <div class="pb-4">{{ $post->user->name }}</div>
                @if(Auth::user())
                <div class="p-4 border-none  rounded-2xl"><a href="/edit/{{ $post->id }}" class="p-3 pl-5 pr-5 border rounded-2xl mr-64  border-slate-700" >Edit</a></div>
